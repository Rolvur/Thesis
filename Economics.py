...
#df_RepWeeks.columns[2]
#df_RepWeeks.columns[3]
#df_RepWeeks.columns[4]
...
avoid = 'year'
find_unique = 'V'
#Import and merge all files in "path" containing "find_year, "find_model" and "find_unique" in file name
...
def import_w_model_results(path,find_year,find_model, find_unique, avoid):
    ...
    for f in files_xls:
        if (find_year in f) and (find_model in f) and (find_unique in f):
            if not f.startswith("~"):
                if not f.endswith('12-31.xlsx'): #OBS!
                    print(f)
...
def calc_vOPEX_year(find_model, find_year,All_Data, All_Pi):
    vOPEX_year = {}
    for m in range(0,len(find_model)):
        for y in range(0,len(find_year)):
            dict_key = 'df_'+find_model[m]+'_'+find_year[y]
            print(dict_key)
            vOPEX_year[dict_key] = {}
            for π in range(0,len(All_Pi[dict_key])):
                print(sum(All_Data[dict_key]['DA_revenue'][π*168:(π+1)*168])*All_Pi[dict_key][π]*(365/7))
            ...
            vOPEX_year[dict_key]['DA_revenue_old'] = sum(All_Data[dict_key]['DA_revenue'])*((365*24)/len(All_Data[dict_key]))
...
vOPEX_year = calc_vOPEX_year(find_model, find_year,All_Data, All_Pi)
...
for m in range(0,len(find_model)):
    for y in range(0,len(find_year)):
        ...
        df_Econ.to_excel(writer, sheet_name=dict_key)
writer.save()
