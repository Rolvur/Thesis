...
def BootsrapFCR(FCR,n_samples,blocksize,sample_length):
...
def KMedoiddsFCR(n_clusters,FCR_boot):

    kmedoids = KMedoids(n_clusters=n_clusters,metric='euclidean').fit(FCR_boot)


    ## Calculating scenario probability ## 

    FCR_medoids = kmedoids.cluster_centers_ 

    Prob = np.zeros((n_clusters,1))

    for j in range(0,n_clusters):
        Prob[j] = np.count_nonzero(kmedoids.labels_ == j)/len(kmedoids.labels_)


    return FCR_medoids, Prob
...
if Type == 'combined':
    ...
    Data.remove(Data[1])
    Data_names.remove(Data_names[1])
...
FCR_list = Data[1]
FCR_boot = BootsrapFCR(FCR_list,n_samples,blocksize,sample_length)

FCR_med, Prob = KMedoiddsFCR(n_clusters,FCR_boot)
