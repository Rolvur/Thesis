...
k_eff =1.05 #0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15, 1.20
#Raw methanol storage:
k_Storage =1.0 #0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15, 1.20
#Methanol demand:
k_demand =1.0 #0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15, 1.20
Demand_scaled = Demand.copy()
for i in Demand.keys():
  Demand_scaled[i] = Demand[i]*k_demand
#------------------------------------------
...
model.m_demand = pe.Param(model.T, initialize = Demand_scaled)
...
model.r_in = r_in
model.r_out = r_out
model.k_d = k_d*k_demand #sensitivity
model.S_Pu_max = S_Pu_max*k_demand #sensitivity
model.S_raw_max = S_raw_max*k_Storage #sensitivity
...
model.m_H2 = pe.Var(model.T, domain=pe.NonNegativeReals, bounds=(0,1500)) #sensitivity gives "fake limit" when increasing efficiency
...
df_results.to_excel("Result_files/Sens_eff_105_V2_year_"+Start_date[:10]+"_"+End_date[:10]+ ".xlsx")
...
with open("Result_files/Sens_eff_105_V2_Parameters_"+Start_date[:10]+"_"+End_date[:10]+ ".csv", 'w', newline='') as csvfile:
